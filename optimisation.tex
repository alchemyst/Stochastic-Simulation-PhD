...
It has also been proposed \citehere\ that NFL's corollary is that
...
\begin{quote}
  A solution \varx{1} is said to dominate the other solution \varx{2},
  if both conditions 1 and 2 are true:
  \begin{enumerate}
  \item The solution \varx{1} is no worse than \varx{2} in all
    objectives, or $f_j(\varx{1}) \ntriangleright f_j(\varx{2})$ for
    all $j = 1,2,\ldots,M$
  \item The solution \varx{1} is strictly better than \varx{2} in at
...
This ``weak formulation'' may be contrasted witht the ``strong'' or
